...
func TestSimulationInboundCheckDepositAdjustmentWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Simulations.InboundCheckDeposits.Adjustment(
		...
		increase.SimulationInboundCheckDepositAdjustmentParams{
			Amount: increase.F(int64(1000)),
			Reason: increase.F(increase.SimulationInboundCheckDepositAdjustmentParamsReasonLateReturn),
		},
	)
	if err != nil {
		var apierr *increase.Error
