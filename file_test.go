...
func TestFileListWithOptionalParams(t *testing.T) {
	...
	_, err := client.Files.List(context.TODO(), increase.FileListParams{
		...
		Purpose: increase.F(increase.FileListParamsPurpose{
			In: increase.F([]increase.FileListParamsPurposeIn{increase.FileListParamsPurposeInCardDisputeAttachment}),
		}),
	})
	if err != nil {
		var apierr *increase.Error
...
func TestFileContents(t *testing.T) {
	...
	resp, err := client.Files.Contents(context.TODO(), "file_makxrc67oh9l6sg7w9yc")
	if err != nil {
		var apierr *increase.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
