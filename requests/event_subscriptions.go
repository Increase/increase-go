...
type EventSubscription struct {
	...
	Status fields.Field[EventSubscriptionStatus] `json:"status,required"`
	// If specified, this subscription will only receive webhooks for Events with the
	// specified `category`.
	SelectedEventCategory fields.Field[EventSubscriptionSelectedEventCategory] `json:"selected_event_category,required,nullable"`
	// The webhook url where we'll send notifications.
	URL fields.Field[string] `json:"url,required"`
	// The key that will be used to sign webhooks.
	SharedSecret fields.Field[string] `json:"shared_secret,required"`
	// A constant representing the object's type. For this resource it will always be
	// `event_subscription`.
	Type fields.Field[EventSubscriptionType] `json:"type,required"`
}
...
func (r EventSubscription) String() (result string) {
	return fmt.Sprintf("&EventSubscription{ID:%s CreatedAt:%s Status:%s SelectedEventCategory:%s URL:%s SharedSecret:%s Type:%s}", r.ID, r.CreatedAt, r.Status, r.SelectedEventCategory, r.URL, r.SharedSecret, r.Type)
}
