...
func TestCardProfilesListWithOptionalParams(t *testing.T) {
	c := increase.NewIncrease(options.WithAPIKey("APIKey"), options.WithBaseURL("http://127.0.0.1:4010"))
	_, err := c.CardProfiles.List(context.TODO(), &types.CardProfileListParams{Cursor: increase.P("string"), Limit: increase.P(int64(0)), Status: &types.CardProfilesListParamsStatus{In: &[]types.CardProfilesListParamsStatusIn{types.CardProfilesListParamsStatusInPending, types.CardProfilesListParamsStatusInPending, types.CardProfilesListParamsStatusInPending}}})
	if err != nil {
		t.Fatal("err should be nil", err)
	}
}
